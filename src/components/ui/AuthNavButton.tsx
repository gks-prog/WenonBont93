...
export function AuthNavButton() {
  ...
  if (user) {
    ...
    return (
      <a 
        href="/dashboard" 
        // CRITICAL FIX: z-[9999] forces this button to the front layer so it cannot be blocked
        className="relative z-[9999] group block cursor-pointer"
      >
  ...
  return (
    <a 
      href="/login"
      // CRITICAL FIX: z-[9999]
      className="relative z-[9999] px-5 py-2 text-[10px] uppercase tracking-[0.2em] font-bold text-white border border-white/20 rounded-sm hover:bg-white hover:text-black transition-all cursor-pointer"
    >
      Login
    </a>
  );
